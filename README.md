...
也可以下载 `dsh-custom-distribution-v0.5.1-win-x64.zip`，解压后运行：
...
- `.credentials.yaml`、API key、OAuth token、Cookie；其中 rc2 首次启动会在本机创建仅用于浏览器 Cookie 签名的内部 grant，但发行包不会预置、读取或上传它；
...
OpenCode Go 会话路由修复已纳入 v0.5.1，详见 [发行说明](docs/RELEASE-0.5.1.md)。
