...
var ClientModuleRegistry = class extends Service {
	...
	*/
	...
	nearestPackage(moduleUrl, expectedPackageName, visited = new Set()) {
		if (visited.has(moduleUrl)) throw new Error("client-modules: cyclic module fallback");
		...
		while (true) {
			...
			if (existsSync(candidate)) try {
				const packageRecord = JSON.parse(readFileSync(candidate, "utf8"));
                const name = packageRecord.name;
                const proxyTarget = packageRecord.dsh?.moduleFallback?.targets?.["."];
                if (typeof name === "string" && (expectedPackageName === void 0 || name === expectedPackageName) && typeof proxyTarget === "string" && proxyTarget.startsWith("file:")) {
                    return this.nearestPackage(proxyTarget, name, visited);
                }
