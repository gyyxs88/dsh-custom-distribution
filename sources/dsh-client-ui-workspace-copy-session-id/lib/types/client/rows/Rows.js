...
import { HoverCard, IconAlarmClockOutline16, IconArchiveOutline20, IconBranchOutline16, IconCheckOutline16, IconCopyOutline16, IconEditOutline16, IconEllipsisOutline16, IconFolderClose16, IconFolderOpen16, IconWarningOutline16, IconPlusOutline16, IconTrashOutline16, IconTriangleRightFill14, Menu, relativeTime, StateDot, Toast, writeClipboard, } from '@deepseek-ai/dsh-client-ui-primitives';
...
export function SessionNodeItem({ node, currentId, now, onOpen, onRename, onFork, onArchive, drag, flat = false, t }) {
    ...
    const copyAnchor = useRef(null);
    const copyEpoch = useRef(0);
    useEffect(() => () => { copyEpoch.current += 1; }, []);
    // Archive hides the row through the registry-global archive set and never
    // touches the session log, so it is not styled as destructive and needs no
    // confirmation dialog.
    const sessionMenuItems = [
        { id: 'rename', label: t('rename'), icon: _jsx(IconEditOutline16, {}) },
        { id: 'copy-session-id', label: t('menu.copySessionId'), icon: _jsx(IconCopyOutline16, {}) },
    ...
    const ownRow = (_jsxs("div", { className: clsx(css.sessionRow, selected && css.selected, menuOpen && css.menuOpen, flat && !showStatus && css.flatSessionRowWithoutStatus, drag?.marker === 'before' && css.dropBefore, drag?.marker === 'after' && css.dropAfter), role: "treeitem", "aria-selected": selected, onClick: () => { onOpen(node.id); }, draggable: drag !== undefined, onDragStart: drag === undefined
            ...
            : (e) => {
                ...
                e.preventDefault();
                drag.drop(rowHalf(e));
            }, children: [(!flat || showStatus) && (_jsx("span", { className: css.slot, children: showStatus && _jsx(SessionStatusDots, { statuses: statuses }) })), _jsx("span", { className: css.title, children: title }), row.hasActiveSchedule && _jsx(ActiveScheduleIndicator, { t: t }), !row.blank && _jsx("span", { className: css.time, children: timeLabel(row.updatedAt, now, t) }), !row.blank && (_jsx("span", { className: css.rowActions, children: _jsxs(_Fragment, { children: [_jsx(Menu, { open: menuOpen, onClose: () => { setMenuOpen(false); }, items: sessionMenuItems, onSelect: (id) => {
                                setMenuOpen(false);
                                if (id === 'copy-session-id') {
                                    const epoch = ++copyEpoch.current;
                                    setCopyStatus(null);
                                    void writeClipboard(node.id).then(ok => { if (epoch === copyEpoch.current)
                                        setCopyStatus(ok ? 'copied' : 'failed'); }, () => { if (epoch === copyEpoch.current)
                                ...
                                if (id === 'archive')
                                    onArchive(node.id);
                            }, portal: true, closeOnPointerLeave: true, anchor: (_jsx("button", { ref: copyAnchor, type: "button", className: css.iconButton, "aria-label": t('actions.session.aria', { name: title }), onClick: (e) => { e.stopPropagation(); setMenuOpen(v => !v); }, children: _jsx(IconEllipsisOutline16, {}) })) }), copyStatus === null
                            ? null
                            : (_jsx(Toast, { text: t(copyStatus === 'copied'
                                    ? 'menu.copySessionId.copied'
                                    : 'menu.copySessionId.failed'), icon: copyStatus === 'copied' ? _jsx(IconCheckOutline16, {}) : _jsx(IconWarningOutline16, {}), anchor: copyAnchor.current, onDone: () => { setCopyStatus(null); } }))] }) }))] }));
