...
export interface LlmDiscoveredModel {
    ...
    maxTokens?: number;
    /** Request modalities explicitly advertised by the endpoint. */
    input?: readonly ModelModality[];
    /** Endpoint-advertised selectable reasoning levels and their wire spellings. */
    reasoningEfforts?: false | Partial<Record<'off' | 'minimal' | 'low' | 'medium' | 'high' | 'xhigh' | 'max', string | null>>;
    /** Endpoint-advertised default reasoning level, only when it is also offered. */
