...
// Exact local producer schemas from the preserved 0.1.1 deployment. No opaque payloads are rewritten.
function localPluginSourceValue(source, label) {
 const schemas = {
  "dsh-session-control": {forms:["relay","operation-terminal-report","operation-attention-report","permission-change","approval-notice"], fields:["provenanceVersion","senderDisplayName","senderSessionId","senderSessionTitle","targetSessionId","operationId"]},
  "dsh-subagent-code-agents": {forms:["run-terminal-report","run-interrupted-report"], fields:["provenanceVersion","senderDisplayName","senderSessionTitle","targetSessionId","runId","channel","jobId"]},
  "session-control": {forms:["relay"], fields:["senderSessionId","traceId"]}
 };
 const schema=schemas[source.plugin];
 if(!schema)return false;
 assertReleasedV0Keys(source,["kind","plugin","form"],schema.fields,label);
 literalValue(source.form,schema.forms,label+" form");
 for(const key of schema.fields)if(source[key]!==void 0){
  if(key==="provenanceVersion")literalValue(source[key],[1],label+" provenanceVersion");
  else stringValue(source[key],label+" "+key);
 }
 return true;
}

function pluginSourceValue(source, label) {
	if (localPluginSourceValue(source, label)) return;
	const optional = [
		"form",
		"sections",
		"summary"
...
function assertReleasedEventPayload(event, version) {
	...
	const data = releasedV0Record(event.data, `${event.type} ${event.seq} data`);
	if (event.type === "subagent/descriptor" && data["version"] === 2 && version === 0) {
  assertReleasedV0Keys(data,["version","mode","provider"],["label","agentProvider","agentModel","persona","toolFilter"],"legacy descriptor v2");
  subagentDescriptorValue({...data,version:3},"legacy descriptor v2");
  return;
 }
...
var ReleasedV0ToV1Stage = class {
	...
	transformEvent(event, context) {
		...
		context.emitEvent(normalized.type === "subagent/descriptor" && normalized.data.version === 2 ? {...normalized,data:{...normalized.data,version:3}} : normalized);
	}
