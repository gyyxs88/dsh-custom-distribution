...
* dependency closure through Node's ordinary parent-walk. Plain Node normally
* uses symlinks for that shared fallback; packaged executables and Windows
* distributions that explicitly request it use ESM proxies so external
* plugins retain the installation's module instances without depending on
* junction traversal.
...
/** Opt-in used by portable Windows distributions whose install volume cannot traverse junctions. */
const MODULE_FALLBACK_MODE_ENV = "DSH_MODULE_FALLBACK_MODE";
/**
...
/** Select real ESM proxies when the executable or its trusted launcher requires them. */
function useModuleProxies() {
	const mode = process.env[MODULE_FALLBACK_MODE_ENV];
	if (mode !== void 0 && mode !== "proxy") throw new Error(`dsh: ${MODULE_FALLBACK_MODE_ENV} must be "proxy" when set`);
	return isPackagedExecutable() || mode === "proxy";
}
...
function resolveModuleFallbackEntries(installAnchor) {
 ...
 /* v8 ignore next -- a real app manifest always declares dependencies */
	...
	return {
		entries: !useModuleProxies() ? [...links].map(([packageName, packageDir]) => ({
			kind: "symlink",
			packageName,
			packageDir
...
* closure. Plain Node writes symlinks by default; a packaged executable or a
* launcher setting `DSH_MODULE_FALLBACK_MODE=proxy` writes ESM proxies under a
* cross-process lock. Missing packages carried only by selected
