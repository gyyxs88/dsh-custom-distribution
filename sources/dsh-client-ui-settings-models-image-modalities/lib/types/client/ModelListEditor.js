...
function adopt(candidate) {
    return {
        ...
        ...candidate.reasoningEfforts === undefined
            ? {}
            : { reasoningEfforts: candidate.reasoningEfforts === false ? false : { ...candidate.reasoningEfforts } },
...
function mergeCandidate(existing, candidate) {
    ...
    const next = { ...existing };
    for (const field of [
        'name', 'contextWindow', 'maxTokens', 'input', 'reasoningEfforts', 'defaultReasoningEffort',
    ]) {
        if (next[field] === undefined && found[field] !== undefined)
...
export function ModelListEditor(props) {
    ...
    const protocol = probe.api ?? (probe.provider === 'openrouter' ? 'openai-completions' : undefined);
    ...
    const reasoningMode = (model) => model['reasoningEfforts'] === false
        ? 'none'
    ...
    const setReasoningMode = (index, mode) => {
        if (mode === 'auto')
            patch(index, { reasoningEfforts: undefined, defaultReasoningEffort: undefined });
        else if (mode === 'none')
            patch(index, { reasoningEfforts: false, defaultReasoningEffort: undefined });
        else
            patch(index, { reasoningEfforts: { off: null, low: 'low', medium: 'medium', high: 'high' } });
    };
    const setReasoningLevel = (model, index, level, enabled) => {
        ...
        if (enabled)
            ...
            Reflect.deleteProperty(efforts, level);
        patch(index, {
            reasoningEfforts: efforts,
            ...!enabled && model['defaultReasoningEffort'] === level ? { defaultReasoningEffort: undefined } : {},
        });
    };
    const setCompat = (model, index, field, value) => {
        const compat = { ...objectValue(model['compat']) };
        if (value === undefined || value === '')
            Reflect.deleteProperty(compat, field);
        else
            compat[field] = value;
        patch(index, { compat: Object.keys(compat).length === 0 ? undefined : compat });
    };
    const triStateCompat = (model, index, field, label) => {
        const value = objectValue(model['compat'])[field];
        return (_jsxs("label", { className: styles['modelField'], children: [_jsx("span", { className: styles['modelFieldLabel'], children: t(label) }), _jsxs("select", { className: `${styles['input']} ${styles['selectInput']}`, "aria-label": t(label), value: typeof value === 'boolean' ? String(value) : '', disabled: disabled, onChange: (event) => {
                        setCompat(model, index, field, event.target.value === '' ? undefined : event.target.value === 'true');
                    }, children: [_jsx("option", { value: "", children: t('automatic') }), _jsx("option", { value: "true", children: t('enabled') }), _jsx("option", { value: "false", children: t('disabled') })] })] }));
    };
    const renderCapabilities = (model, index) => {
        const modalities = modalitiesOf(model);
        const mode = reasoningMode(model);
        const efforts = objectValue(model['reasoningEfforts']);
        return (_jsxs(_Fragment, { children: [_jsxs("div", { className: styles['modelField'], style: { gridColumn: '1 / -1' }, children: [_jsx("span", { className: styles['modelFieldLabel'], children: t('inputCapabilities') }), _jsx("span", { children: ['text', 'image'].map(modality => (_jsxs("label", { style: { marginRight: '16px' }, children: [_jsx("input", { type: "checkbox", checked: modalities.includes(modality), disabled: disabled, onChange: (event) => { setModality(model, index, modality, event.target.checked); } }), " ", t(modality === 'text' ? 'inputText' : 'inputImage')] }, modality))) })] }), _jsxs("label", { className: styles['modelField'], children: [_jsx("span", { className: styles['modelFieldLabel'], children: t('reasoningCapability') }), _jsxs("select", { className: `${styles['input']} ${styles['selectInput']}`, "aria-label": t('reasoningCapability'), value: mode, disabled: disabled, onChange: (event) => { setReasoningMode(index, event.target.value); }, children: [_jsx("option", { value: "auto", children: t('reasoningAuto') }), _jsx("option", { value: "none", children: t('reasoningNone') }), _jsx("option", { value: "custom", children: t('reasoningCustom') })] })] }), mode !== 'custom' ? null : (_jsxs("div", { className: styles['modelField'], style: { gridColumn: '1 / -1' }, children: [_jsx("span", { className: styles['modelFieldLabel'], children: t('reasoningLevels') }), REASONING_LEVELS.map(level => (_jsxs("label", { style: { display: 'grid', gridTemplateColumns: '100px minmax(160px, 1fr)', gap: '8px' }, children: [_jsxs("span", { children: [_jsx("input", { type: "checkbox", "aria-label": `${t('reasoningLevels')} ${level}`, checked: Object.prototype.hasOwnProperty.call(efforts, level), disabled: disabled, onChange: (event) => { setReasoningLevel(model, index, level, event.target.checked); } }), " ", level] }), level === 'off'
                                    ? _jsx("span", { className: styles['advancedHint'], children: t('reasoningOffWire') })
                                    : (_jsx("input", { className: styles['input'], type: "text", value: typeof efforts[level] === 'string' ? efforts[level] : level, disabled: disabled || !Object.prototype.hasOwnProperty.call(efforts, level), "aria-label": `${t('reasoningWireValue')} ${level}`, onChange: (event) => {
                                            patch(index, { reasoningEfforts: { ...efforts, [level]: event.target.value } });
                                        } }))] }, level))), _jsxs("label", { className: styles['modelField'], children: [_jsx("span", { className: styles['modelFieldLabel'], children: t('reasoningDefault') }), _jsxs("select", { className: `${styles['input']} ${styles['selectInput']}`, value: typeof model['defaultReasoningEffort'] === 'string'
                                        ? model['defaultReasoningEffort']
                                        : '', "aria-label": t('reasoningDefault'), disabled: disabled, onChange: (event) => {
                                        patch(index, { defaultReasoningEffort: event.target.value || undefined });
                                    }, children: [_jsx("option", { value: "", children: t('providerDefault') }), REASONING_LEVELS.filter(level => Object.prototype.hasOwnProperty.call(efforts, level))
                                            .map(level => _jsx("option", { value: level, children: level }, level))] })] })] }))] }));
    };
    const renderModelCompat = (model, index) => {
        if (protocol === undefined) {
            return _jsx("p", { className: styles['advancedHint'], style: { gridColumn: '1 / -1' }, children: t('compatNeedsProtocol') });
        }
        const compat = objectValue(model['compat']);
        if (protocol === 'openai-completions') {
            return (_jsxs(_Fragment, { children: [_jsx("h4", { style: { gridColumn: '1 / -1', margin: '8px 0 0', fontSize: '12px' }, children: t('compatibility') }), _jsxs("label", { className: styles['modelField'], children: [_jsx("span", { className: styles['modelFieldLabel'], children: t('thinkingFormat') }), _jsxs("select", { className: `${styles['input']} ${styles['selectInput']}`, value: typeof compat['thinkingFormat'] === 'string' ? compat['thinkingFormat'] : '', disabled: disabled, onChange: (event) => { setCompat(model, index, 'thinkingFormat', event.target.value || undefined); }, children: [_jsx("option", { value: "", children: t('automatic') }), THINKING_FORMATS.map(value => _jsx("option", { value: value, children: value }, value))] })] }), _jsxs("label", { className: styles['modelField'], children: [_jsx("span", { className: styles['modelFieldLabel'], children: t('maxTokensField') }), _jsxs("select", { className: `${styles['input']} ${styles['selectInput']}`, value: typeof compat['maxTokensField'] === 'string' ? compat['maxTokensField'] : '', disabled: disabled, onChange: (event) => { setCompat(model, index, 'maxTokensField', event.target.value || undefined); }, children: [_jsx("option", { value: "", children: t('automatic') }), MAX_TOKEN_FIELDS.map(value => _jsx("option", { value: value, children: value }, value))] })] }), triStateCompat(model, index, 'supportsReasoningEffort', 'supportsReasoningEffort'), triStateCompat(model, index, 'supportsDeveloperRole', 'supportsDeveloperRole'), triStateCompat(model, index, 'supportsStrictMode', 'supportsStrictTools')] }));
        }
        if (protocol === 'anthropic-messages') {
            return _jsxs(_Fragment, { children: [triStateCompat(model, index, 'supportsTemperature', 'supportsTemperature'), triStateCompat(model, index, 'forceAdaptiveThinking', 'forceAdaptiveThinking'), triStateCompat(model, index, 'supportsStrictTools', 'supportsStrictTools')] });
        }
        if (protocol.includes('responses')) {
            return _jsxs(_Fragment, { children: [triStateCompat(model, index, 'supportsDeveloperRole', 'supportsDeveloperRole'), triStateCompat(model, index, 'supportsStrictMode', 'supportsStrictTools')] });
        }
        return null;
    };
    ...
    return (_jsxs("section", { className: styles['modelCatalog'], "aria-label": t('models'), children: [_jsxs("div", { className: styles['modelListHead'], children: [_jsxs("div", { className: styles['modelCatalogHeading'], children: [_jsx("span", { className: styles['modelCatalogTitle'], children: t('models') }), props.overridden === undefined
                        ...
                        : null, _jsx("button", { type: "button", className: styles['linkButton'], disabled: disabled || busy || !askable || props.probeBlocked !== undefined, title: props.probeBlocked !== undefined
                            ...
                            : askable ? undefined : t('fetchNeedsBaseUrl'), onClick: () => { void fetchModels(); }, children: busy ? t('fetching') : t('fetchModels') })] }), models.length === 0 ? _jsx("p", { className: styles['modelEmpty'], children: t('modelsEmpty') }) : null, models.map((model, index) => (_jsxs("div", { className: styles['modelEntry'], children: [_jsxs("div", { className: styles['modelRow'], style: { gridTemplateColumns: 'minmax(0,1.4fr) minmax(0,1fr) auto auto auto' }, children: [_jsx("input", { className: styles['input'], type: "text", value: textOf(model, 'id'), placeholder: t('modelId'), "aria-label": `${t('modelId')} ${index + 1}`, disabled: disabled, onChange: (event) => { patch(index, { id: event.target.value }); } }), _jsx("input", { className: styles['input'], type: "text", value: textOf(model, 'name'), placeholder: t('modelName'), "aria-label": `${t('modelName')} ${index + 1}`, disabled: disabled, onChange: (event) => { patch(index, { name: event.target.value === '' ? undefined : event.target.value }); } }), _jsx("span", { className: styles['rowTag'], children: t(model['source'] === 'discovered' ? 'modelSourceDiscovered' : 'modelSourceManual') }), _jsx("button", { type: "button", className: styles['iconButton'], "aria-label": `${t('modelAdvanced')} ${index + 1}`, "aria-expanded": expanded.has(index), title: t('modelAdvanced'), onClick: () => { toggleExpanded(index); }, children: _jsx(IconChevron, { open: expanded.has(index) }) }), _jsx("button", { type: "button", className: `${styles['iconButton']} ${styles['iconButtonDanger']}`, "aria-label": `${t('removeModel')} ${index + 1}`, title: t('removeModel'), disabled: disabled, onClick: () => {
