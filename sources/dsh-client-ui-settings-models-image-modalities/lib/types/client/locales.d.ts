/** Copy dictionaries for the Models settings section. */
/** English strings (the key-set source of truth for this pair). */
export declare const en: {
    nav: string;
    title: string;
    intro: string;
    edit: string;
    editProvider: string;
    remove: string;
    removeProvider: string;
    deleteTitle: string;
    deleteDescription: string;
    deleteDescriptionWithCredential: string;
    deleteConfirm: string;
    deleting: string;
    add: string;
    provider: string;
    close: string;
    cancel: string;
    apply: string;
    applying: string;
    savedProvider: string;
    credentialConfigured: string;
    credentialMissing: string;
    readOnly: string;
    loadFailed: string;
    conflict: string;
    retry: string;
    keyInput: string;
    keyPlaceholder: string;
    keyPlaceholderNative: string;
    keyStored: string;
    keyEnvLocked: string;
    customized: string;
    baseUrl: string;
    baseUrlDefault: string;
    models: string;
    modelsInherited: string;
    modelsCustomized: string;
    resetModels: string;
    model: string;
    modelId: string;
    modelName: string;
    modelNamePlaceholder: string;
    contextWindow: string;
    contextWindowPlaceholder: string;
    maxTokens: string;
    maxTokensPlaceholder: string;
    modelAdvanced: string;
    addModel: string;
    removeModel: string;
    modelsEmpty: string;
    keyBlank: string;
    keyBlankNew: string;
    keyIllegalCharacters: string;
    modelIdRequired: string;
    modelIdDuplicate: string;
    modelNameInvalid: string;
    modelContextInvalid: string;
    modelMaxTokensInvalid: string;
    advancedHint: string;
    modelCapacityInvalid: string;
    modelDuplicate: string;
    modelContextWindow: string;
    modelMaxTokens: string;
    modelSourceDiscovered: string;
    modelSourceManual: string;
    inputCapabilities: string;
    inputText: string;
    inputImage: string;
    reasoningCapability: string;
    reasoningAuto: string;
    reasoningNone: string;
    reasoningCustom: string;
    reasoningLevels: string;
    reasoningOffWire: string;
    reasoningWireValue: string;
    reasoningDefault: string;
    providerDefault: string;
    reasoningUnavailable: string;
    reasoningSelectionUnavailable: string;
    automatic: string;
    enabled: string;
    disabled: string;
    compatibility: string;
    compatNeedsProtocol: string;
    thinkingFormat: string;
    maxTokensField: string;
    supportsReasoningEffort: string;
    supportsDeveloperRole: string;
    supportsStrictTools: string;
    supportsTemperature: string;
    forceAdaptiveThinking: string;
    openRouterRouting: string;
    routingOnly: string;
    routingOrder: string;
    routingIgnore: string;
    routingQuantizations: string;
    routingFallbacks: string;
    routingRequireParameters: string;
    routingZdr: string;
    routingDistillable: string;
    routingDataCollection: string;
    routingDataDeny: string;
    routingDataAllow: string;
    routingSort: string;
    routingSortPrice: string;
    routingSortThroughput: string;
    routingSortLatency: string;
    routingMaxPromptPrice: string;
    routingMaxCompletionPrice: string;
    routingMinThroughput: string;
    routingMaxLatency: string;
    defaultModelTitle: string;
    defaultModelDescription: string;
    unavailable: string;
    saveDefault: string;
    modelCatalogPartial: string;
    modelInputInvalid: string;
    modelReasoningInvalid: string;
    modelReasoningDefaultInvalid: string;
    fetchModels: string;
    fetching: string;
    fetchNeedsBaseUrl: string;
    fetchEmpty: string;
    fetchTitle: string;
    fetchDescription: string;
    fetchSelectAll: string;
    fetchDeselectAll: string;
    fetchAdopt: string;
    customAdd: string;
    customTitle: string;
    customTag: string;
    customRoute: string;
    customRouteHint: string;
    customRouteInvalid: string;
    customRouteTaken: string;
    customDisplayName: string;
    customApi: string;
    customApiUnset: string;
    customNeedsBaseUrl: string;
    customNeedsModels: string;
    create: string;
    creating: string;
    welcomeTitle: "Internal Testing Notice";
    welcomeBody: "DeepSeek Harness 0.1 remains in testing for Harness developers. Many areas need further improvement, and we welcome feedback from the developer community. DeepSeek Harness's core plugins and foundational APIs will continue to evolve rapidly over the coming months.\n\nWe look forward to exploring the limits of intelligence with developers around the world, building on open-source, open, reusable, and composable infrastructure. We welcome Harness developers everywhere to join the DSH plugin ecosystem.";
    welcomeContinue: "Continue";
    welcomeError: string;
    onboardingTitle: string;
    onboardingDescription: string;
    onboardingLater: string;
    onboardingSave: string;
    onboardingSaving: string;
    keyRequired: string;
};
/** The settings.models namespace key union. */
export type ModelsKey = keyof typeof en;
/** Chinese strings (same keys as {@link en}). */
export declare const zh: {
    [Key in keyof typeof en]: string;
};
//# sourceMappingURL=locales.d.ts.map
