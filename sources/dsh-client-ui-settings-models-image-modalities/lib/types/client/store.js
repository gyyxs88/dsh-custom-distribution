...
export class ModelsSettingsStore {
    ...
    async load() {
        ...
        const [registered, declared, modelCatalog] = await Promise.all([
            ...
            this.ctx.remote.session?.modelCatalog?.() ?? Promise.resolve(undefined),
        ...
        this.store.update((s) => {
            ...
            s.modelFailures = modelCatalog?.ok === true
                ? modelCatalog.value.failures
                : modelCatalog === undefined ? [] : [{ id: '', name: '', message: modelCatalog.error.message }];
        });
    }
