...
export function validateDeepSeekModels(value) {
    ...
    for (const [index, model] of models.entries()) {
        ...
        if (efforts !== undefined && efforts !== false) {
            ...
            if (!entries.some(([level]) => level !== 'off') || entries.some(([level, wire]) => !REASONING_LEVELS.includes(level)
                || (level === 'off' ? wire !== null : typeof wire !== 'string' || wire.length === 0))) {
                return { index, key: 'modelReasoningInvalid' };
            }
            const defaultEffort = model['defaultReasoningEffort'];
            if (defaultEffort !== undefined && (typeof defaultEffort !== 'string'
                || !Object.prototype.hasOwnProperty.call(efforts, defaultEffort))) {
                return { index, key: 'modelReasoningDefaultInvalid' };
            }
        }
        else if (efforts === false && model['defaultReasoningEffort'] !== undefined) {
            return { index, key: 'modelReasoningDefaultInvalid' };
        }
    }
    return undefined;
