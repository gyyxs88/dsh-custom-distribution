...
同一展开区现在还编辑输入模态、明确的思维能力、各档位的请求值、模型级默认档位和协议兼容性。「自动判断／未知」保留继承或供应商自己的元数据；「不支持思维」写入明确的否定能力；「配置思维档位」要求至少一个非关闭档位，并要求每个已启用档位都有请求值。自动发现只采纳端点明确报告的元数据；刷新只补齐缺失字段，绝不覆盖用户修正。页面另行写入未来会话使用的 `agent-default-model`，并拒绝保存当前目录已经不提供的旧供应商、旧模型或旧思维档位。

对于 `openrouter` 路由，提供方展开区还可编辑受支持的上游路由策略。空列表与空的嵌套策略对象会被省略，从而保留 OpenRouter 自身默认行为，而不是意外发送空允许列表。
...
- **卡片仍是精选编辑器，而不是通用 schema 表单**：目前已经覆盖路由身份、端点、模型容量／模态／思维档位／默认值、常用协议兼容字段和 OpenRouter 路由。重试策略、超时、自定义标头、缓存及其他少用字段仍留在 `settings.yaml`；页面未展示的字段会在每次编辑时保留。不带约定字段的 profile schema 只渲染提示，两套精选布局仍以 `llm-deepseek`/`llm-pi-ai` namespace 名为键。
