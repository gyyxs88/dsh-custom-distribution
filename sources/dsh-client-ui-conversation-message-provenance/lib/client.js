window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		/** Read one non-empty string field without trusting an inherited property. */
		function readString(record, key) {
			const value = Object.hasOwn(record, key) ? record[key] : void 0;
		...
		*/
		function RelayBody({ content, source, t }) {
			...
			const payload = relayPayload(content, source);
			/* v8 ignore next -- contextBody resolves the relay before choosing this body. */
			if (sender === null || payload === null) return (0, react_jsx_runtime.jsx)(OpaqueBody, {
				content,
				source,
				t
			});
			return (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [(0, react_jsx_runtime.jsx)("p", {
				...
				children: relaySourceTitle(source, t)
			}), (0, react_jsx_runtime.jsx)("pre", {
				className: ContextBody_module_css_default.text,
				"data-context-relay-content": true,
				children: boundedText(payload, t)
			})] });
		}
		...
		/** Human-facing relay provenance, always derived from the durable source. */
		function relaySourceTitle(source, t) {
			const record = asRecord(source);
			const sender = record === null ? null : readString(record, "senderDisplayName");
			const title = record === null ? null : readString(record, "senderSessionTitle");
			const display = sender ?? "DSH";
			return title === null
				? t("message.source.relay.other", { sender: display })
				: t("message.source.relay.named", { sender: display, session: title });
		}
		/** Extract a relay's user-facing body while retaining its security envelope. */
		function relayPayload(content, source) {
			if (!Array.isArray(content) || content.length !== 1) return null;
			const block = asRecord(content[0]);
			if (block === null || block["type"] !== "text" || typeof block["text"] !== "string") return null;
			const match = /^<dsh-session-relay>\n([\s\S]+)\n<\/dsh-session-relay>$/u.exec(block["text"]);
			if (match === null) return null;
			let body;
			try {
				body = asRecord(JSON.parse(match[1]));
			} catch {
				return null;
			}
			const record = asRecord(source);
			if (body === null || record === null || body["version"] !== 1 || typeof body["content"] !== "string") return null;
			if (body["sourceSessionId"] !== record["senderSessionId"]) return null;
			if (typeof record["targetSessionId"] === "string" && body["targetSessionId"] !== record["targetSessionId"]) return null;
			if (typeof record["operationId"] === "string" && body["operationId"] !== record["operationId"]) return null;
			return body["content"];
		}
		/** Bounded one-line relay preview for the always-visible collapsed row. */
		function relaySummary(content, source) {
			const payload = relayPayload(content, source);
			if (payload === null) return null;
			const normalized = payload.replace(/\s+/gu, " ").trim();
			if (normalized === "") return null;
			return normalized.length <= 180 ? normalized : `${normalized.slice(0, 179)}…`;
		}
		/** Visible source label for every logged non-user message. */
		function contextSourceTitle(source, provenance, form, t) {
			const record = asRecord(source);
			const kind = record === null ? null : readString(record, "kind");
			if (form === "relay") return relaySourceTitle(source, t);
			if (kind === "agent-instructions") return t("message.source.instructions");
			if (kind === "skill-catalog") return t("message.source.skillCatalog");
			if (kind === "skill-invocation") return t("message.source.skill", { source: provenance.label ?? kind });
			if (kind === "session-reference") return t("message.source.recall");
			if (kind === "plugin") {
				const plugin = record === null ? null : readString(record, "plugin");
				if (plugin === "schedule") return t("message.source.schedule");
				return t("message.source.plugin", { source: provenance.label ?? plugin ?? kind });
			}
			return t("message.source.automatic", { source: provenance.label ?? kind ?? "unknown" });
		}
		...
		*/
		function contextBody(form, props) {
			...
			switch (form) {
				...
				case "relay": {
					const summary = relaySummary(props.content, props.source);
					return relaySender(props.source) === null || summary === null ? opaque : {
						rendered: "relay",
						summary,
		...
		*/
		function ContextInjectionRow({ content, source, provenance, form, t }) {
			...
			const sourceTitle = contextSourceTitle(source, provenance, form, t);
		...
		*/
		...
		const zh = {
			...
			"message.source.relay.named": "由 {sender} 从任务「{session}」发送",
			"message.source.relay.other": "由 {sender} 从另一项任务发送",
			"message.source.schedule": "由定时任务发送",
			"message.source.instructions": "由工作区规则注入",
			"message.source.skillCatalog": "由技能目录注入",
			"message.source.skill": "由技能 {source} 注入",
			"message.source.recall": "由跨会话召回提供",
			"message.source.plugin": "由插件 {source} 注入",
			"message.source.automatic": "由 {source} 自动发送",
		...
		const en = {
			...
			"message.source.relay.named": "Sent by {sender} from task “{session}”",
			"message.source.relay.other": "Sent by {sender} from another task",
			"message.source.schedule": "Sent by a scheduled task",
			"message.source.instructions": "Injected by workspace instructions",
			"message.source.skillCatalog": "Injected by the skill catalog",
			"message.source.skill": "Injected by skill {source}",
			"message.source.recall": "Provided by cross-session recall",
			"message.source.plugin": "Injected by plugin {source}",
			"message.source.automatic": "Sent automatically by {source}",
