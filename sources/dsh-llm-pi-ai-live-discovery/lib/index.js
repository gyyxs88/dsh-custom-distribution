...
function classifyPiAiError(message) {
	...
	if (/\b(?:network(?:_error)?|connection|socket|fetch)\b|\bECONN[A-Z]+\b/i.test(message) || /\b(?:other side closed|HTTP2 request did not get a response|WebSocket closed unexpectedly)\b/i.test(message) || /\bterminated\b|premature close/i.test(message)) return "TRANSPORT";
	return "PI_AI_ERROR";
}
/**
...
async function discoverModels(request, storedProfile) {
	...
	const preferLiveCatalog = installed !== void 0 && liveCatalogBase !== void 0;
	...
	const url = listingUrl(endpointBaseURL, api);
