...
function configuredCompatEntries(compat) {
    return Object.entries(compat ?? {}).flatMap(([field, value]) => {
        // Schemastery expands every nested optional object/array in this block.
        // Strip those synthetic empty members so an untouched OpenRouter editor
        // does not silently become a configured routing policy.
        if (field === 'openRouterRouting' && typeof value === 'object' && value !== null && !Array.isArray(value)) {
            ...
            for (const [key, nested] of Object.entries(value)) {
                ...
                if (typeof nested === 'object' && nested !== null && !Array.isArray(nested)) {
                    const compact = Object.fromEntries(Object.entries(nested).filter(([, item]) => item !== undefined && item !== ''));
...
export function resolveRouteModels(request) {
    ...
    const models = entries.map((entry) => {
        ...
        if (entry.defaultReasoningEffort !== undefined) {
            ...
            if (!resolved.reasoning || !supported.includes(entry.defaultReasoningEffort)) {
                invalid(provider, `model "${entry.id}" defaultReasoningEffort "${entry.defaultReasoningEffort}" is not one of its configured efforts (${supported.join(', ') || 'none'})`);
            }
