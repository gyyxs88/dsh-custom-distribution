...
`defaultReasoningEffort` is model-local and must name one of that exact model's enabled levels. It wins when a request omits an effort, before the provider-level `reasoning` fallback. Invalid defaults are rejected while resolving settings, before any provider I/O.
...
Three kinds of key are refused rather than dropped: one no protocol declares (a misspelling), one pi-ai's installed catalog owns for a named vendor other than the explicitly supported OpenRouter policy (`zaiToolStream`, `deferredToolsMode`, `sessionAffinityFormat`, `supportsOpenAIGrammarTools`, `supportsToolSearch`, `supportsExplicitPromptCacheMode`, `supportsToolReferences`, `vercelGatewayRouting`, `sendSessionAffinityHeaders`), and one written with no value at all (`supportsDeveloperRole:`), which schemastery passes through as null and which would otherwise replace the installed catalog's value with nothing. `compat.openRouterRouting` is supported for the `openrouter` route and carries explicit provider selection, fallback, privacy, price and latency constraints; empty arrays and empty nested objects are removed before dispatch so an untouched form cannot accidentally mean “allow no providers.” The remaining offered set is pinned to pi-ai's four compat types by drift gates, the protocols carrying them are derived from `Model.compat` itself, and each field's type is derived from upstream rather than restated, so an upgrade that adds a field, gives a further protocol a compat type, or widens a value union fails the build until someone classifies it.
